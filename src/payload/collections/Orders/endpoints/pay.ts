...
import type { InvoiceItem } from '../../../payments/types'
...
function generateInvoiceItems(order: Order): InvoiceItem[] {
  const tickets = order?.tickets
    ? (order.tickets as Array<OrderedTicket & { ticket: Ticket }>).map(t => ({
        name: t.ticket.name,
...
async function stripePay(order: Order, payload: Payload, user: User): Promise<string> {
  ...
  if (!user.stripeClientID) {
    ...
    await payload.update({
      collection: 'users',
      id: user.id,
      data: { stripeClientID: customerID },
      depth: 0,
    })
  }
...
async function quickfilePay(order: Order, payload: Payload, user: User): Promise<string> {
  ...
  if (!user.quickfileClientID) {
    ...
    await payload.update({
      collection: 'users',
      id: user.id,
      data: { quickfileClientID: clientID },
      depth: 0,
    })
  }
