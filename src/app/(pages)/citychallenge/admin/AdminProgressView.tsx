...
const badgeClass = (state: ChallengeState): string =>
  state === 'complete'
    ? classes.badgeComplete
    : state === 'in-progress'
    ? classes.badgeInProgress
    : classes.badgeNotStarted
...
const StatusBadge: React.FC<{
  ...
  const label =
    type === 'counter'
      ...
      ? 'Ticked'
      : 'Not ticked'
  return <span className={[classes.badge, badgeClass(status.state)].join(' ')}>{label}</span>
}
            ...
            <ul className={classes.groupList}>
              {group.items.map(status => (
                <li key={status.teamId} className={classes.groupItem}>
                  ...
                  <span className={[classes.badge, badgeClass(status.state)].join(' ')}>
                    {status.maxCount > 1
                      ? `${status.count} / ${status.maxCount}`
                      : status.state === 'complete'
                      ? 'Ticked'
...
const ChallengesTab: React.FC<{
  ...
  const handleSort = (key: ChallengeSortKey) => {
    if (key === sort) setDir(d => (d === 'asc' ? 'desc' : 'asc'))
      ...
      setSort(key)
      setDir(key === 'name' || key === 'zone' ? 'asc' : 'desc')
    }
  }
    ...
    return [...filtered].sort((a, b) => {
      const value = (item: typeof a): number | string => {
        switch (sort) {
          ...
          case 'zone':
            return (item.location.zone ?? '').toLowerCase()
          case 'sortOrder':
    ...
    <div role="tabpanel">
      ...
      <div className={classes.tableWrap}>
        <table className={classes.table}>
          ...
          <tbody>
            {rows.map(({ location, breakdown }, index) => {
                    ...
                    <tr className={classes.detailRow}>
                      <td colSpan={9}>
                        {teams.length === 0 ? (
                          ...
                          <ChallengeGroups breakdown={breakdown} teamsById={teamsById} />
                        )}
                      </td>
                    </tr>
                  )}
...
export const AdminProgressView: React.FC<{ data: AdminProgressData }> = ({ data }) => {
  const [tab, setTab] = useState<Tab>('teams')

  const prepared = useMemo(() => data.teams.map(prepareTeam), [data.teams])
  ...
  return (
    <div className={classes.view}>
      ...
      {tab === 'teams' ? (
        <TeamsTab teams={prepared} locations={data.locations} />
      ) : (
        <ChallengesTab locations={data.locations} teams={prepared} teamsById={teamsById} />
      )}
    </div>
  )
}
